#!/usr/bin/env python3
"""
Convert parade_2025.csv to YAML format for easier data management
"""
...
def format_yaml_value(value):
    ...
    if value is None:
        ...
        if '\n' in value or len(value) > 80:
            # Use literal block scalar for long/multiline text
            lines = value.replace('\r\n', '\n').replace('\r', '\n').split('\n')
            result = "|\n"
            for line in lines:
                result += f"    {line}\n"
...
def convert_csv_to_yaml():
    ...
    yaml_file = Path("_data/parade_2025.yml")
    ...
    # Write to YAML file manually (without PyYAML dependency)
    with open(yaml_file, 'w', encoding='utf-8') as f:
        f.write("# Oakland Pride 2025 Parade Contingents\n")
        f.write("# Converted from CSV format\n\n")
        
        for i, contingent in enumerate(contingents):
            f.write(f"- # Contingent {contingent.get('order', i+1)}\n")
            for key, value in contingent.items():
    ...
    print(f"Successfully converted {len(contingents)} contingents to {yaml_file}")
    print(f"YAML file is ready at: {yaml_file}")
